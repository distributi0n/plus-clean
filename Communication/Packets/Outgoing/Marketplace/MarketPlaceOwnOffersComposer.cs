namespace Plus.Communication.Packets.Outgoing.Marketplace
{
    ...
    {
        public MarketPlaceOwnOffersComposer(int UserId) : base(ServerPacketHeader.MarketPlaceOwnOffersMessageComposer)
        {
            var i = 0;
            ...
            using (var dbClient = PlusEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                dbClient.SetQuery("SELECT SUM(asking_price) FROM catalog_marketplace_offers WHERE state = '2' AND user_id = '" +
                                  UserId + "'");
                ...
                if (table != null)
                {
                    ...
                    {
                        var num2 = Convert.ToInt32(
                            Math.Floor(((double) row["timestamp"] + 172800.0 - PlusEnvironment.GetUnixTimestamp()) / 60.0));
