namespace Plus.Communication.Packets.Outgoing.Moderation
{
    ...
    {
        public ModeratorUserInfoComposer(DataRow User, DataRow Info) : base(ServerPacketHeader.ModeratorUserInfoMessageComposer)
        {
            ...
            WriteInteger(User != null
                ? Convert.ToInt32(Math.Ceiling((PlusEnvironment.GetUnixTimestamp() - Convert.ToDouble(User["account_created"])) /
                                               60))
                : 0);
            WriteInteger(User != null
                ? Convert.ToInt32(Math.Ceiling((PlusEnvironment.GetUnixTimestamp() - Convert.ToDouble(User["last_online"])) / 60))
                : 0);
            WriteBoolean(User != null
                ? PlusEnvironment.GetGame().GetClientManager().GetClientByUserID(Convert.ToInt32(User["id"])) != null
                : false);
            ...
            WriteString(Convert.ToDouble(Info["trading_locked"]) != 0
