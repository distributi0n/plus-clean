namespace Plus.HabboHotel.Items
{
    ...
    {
        public static Item CreateSingleItemNullable(ItemData Data,
            Habbo Habbo,
            string ExtraData,
            string DisplayFlags,
            int GroupId = 0,
        ...
        public static Item CreateSingleItem(ItemData Data,
            Habbo Habbo,
            string ExtraData,
            string DisplayFlags,
            int ItemId,
        ...
        public static Item CreateGiftItem(ItemData Data,
            Habbo Habbo,
            string ExtraData,
            string DisplayFlags,
            int ItemId,
        ...
        public static List<Item> CreateMultipleItems(ItemData Data, Habbo Habbo, string ExtraData, int Amount, int GroupId = 0)
        {
            ...
            using (var dbClient = PlusEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                for (var i = 0; i < Amount; i++)
                {
                    ...
                    var Item = new Item(Convert.ToInt32(dbClient.InsertQuery()), 0, Data.Id, ExtraData, 0, 0, 0, 0, Habbo.Id,
                        GroupId, 0, 0, "");
                    if (GroupId > 0)
                    {
                        dbClient.SetQuery("INSERT INTO `items_groups` (`id`, `group_id`) VALUES (@id, @gid)");
